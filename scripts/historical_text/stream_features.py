...
download one raw filing -> parse sections -> compute keyword counts -> keep compact evidence snippets -> embed text -> discard raw text
...
from src.filings.topics import topic_counts_for_text, topic_evidence_snippets  # noqa: E402
...
def main() -> None:
    ...
    section_snippets = state.section_snippets.to_dict("records")
    filing_index = state.filing_index.to_dict("records")
    failures = state.failures.to_dict("records")
    existing_snippet_accessions = (
        set(state.section_snippets["accession_no"].astype(str))
        if not state.section_snippets.empty and "accession_no" in state.section_snippets.columns
        else set()
    )
    ...
    processed_filings = 0
    checkpoint_every = max(1, int(args.checkpoint_every_filings))
    for ticker_number, record in enumerate(metadata.itertuples(index=False), start=1):
        ...
        for filing in filings.itertuples(index=False):
            ...
            is_existing_accession = accession_no in existing_accessions
            needs_snippet_refresh = bool(
                args.snippets
                and args.refresh_snippets
                and accession_no not in existing_snippet_accessions
            )
            if is_existing_accession and not needs_snippet_refresh:
                continue
            try:
            ...
            for section_key, span in parsed.sections.items():
                ...
                if args.snippets and (not is_existing_accession or needs_snippet_refresh):
                    for snippet in topic_evidence_snippets(
                        text,
                        max_snippets=args.snippets_per_section,
                        window_chars=args.snippet_window_chars,
                    ):
                        snippet_row = dict(section_metadata)
                        snippet_row.update(snippet)
                        section_snippets.append(snippet_row)
                if is_existing_accession:
                    continue

                count_row = dict(section_metadata)
            ...
            if args.embed and len(embed_buffer) >= args.embedding_batch_size:
                ...
                embed_buffer = []
                write_outputs(output_dir, filing_index, section_counts, embeddings, section_snippets, failures, args)

            processed_filings += 1
            if processed_filings % checkpoint_every == 0:
                write_outputs(output_dir, filing_index, section_counts, embeddings, section_snippets, failures, args)
    ...
    log(
        ...
        f"{len(section_snippets)} snippet rows.",
        tag="text-stream",
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.set_defaults(embed=True, snippets=True)
    parser.add_argument(
        "--refresh-snippets",
        action="store_true",
        help="Re-download existing filings to backfill snippets without recomputing embeddings.",
    )
    parser.add_argument("--snippets-per-section", type=int, default=2)
    parser.add_argument("--snippet-window-chars", type=int, default=420)
...
def write_outputs(
    ...
    embeddings: list[dict[str, object]],
    section_snippets: list[dict[str, object]],
    ...
    if section_snippets:
        pd.DataFrame(section_snippets).drop_duplicates(
            ["accession_no", "section", "snippet_rank"],
            keep="last",
        ).to_parquet(
            output_dir / "historical_section_snippets.parquet",
            index=False,
        )
    if failures:
    ...
    manifest = {
        ...
        "snippets": bool(args.snippets),
        "snippets_per_section": int(args.snippets_per_section),
        ...
        "snippet_rows": len(
            {
                (str(row["accession_no"]), str(row["section"]), int(row.get("snippet_rank", 0)))
                for row in section_snippets
            }
        ),
...
class ExistingState:
    ...
    def __init__(
        ...
        section_snippets: pd.DataFrame,
        failures: pd.DataFrame,
    ) -> None:
        self.filing_index = filing_index
        self.section_counts = section_counts
        self.section_embeddings = section_embeddings
        self.section_snippets = section_snippets
        self.failures = failures

    @classmethod
    def empty(cls) -> "ExistingState":
        return cls(pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame())
...
def load_existing_state(output_dir: Path) -> ExistingState:
    ...
    snippets = pd.read_parquet(snippets_path) if snippets_path.exists() else pd.DataFrame()
    failures = (
        pd.read_json(failures_path, orient="records", lines=True)
        if failures_path.exists()
        else pd.DataFrame()
    )
    return ExistingState(filing_index, counts, embeddings, snippets, failures)
